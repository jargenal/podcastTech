...
from app.utils.text_processing import TextProcessingDebug, adapt_text_to_speech_spans, normalize_text, render_speech_spans
...
def parse_input_document(
    ...
    *,
    ...
    if not _has_tags(text):
        spans = adapt_text_to_speech_spans(normalize_text(text), settings, default_language=block_language, debug=debug)
    ...
    def flush() -> None:
        ...
        if current_tag == "TITULO" and value:
            ...
            spans = adapt_text_to_speech_spans(normalize_text(value), settings, default_language=block_language, debug=debug)
